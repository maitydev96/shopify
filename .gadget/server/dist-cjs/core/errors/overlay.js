...
_export(exports, {
    ChevronRightIconUrl: function() {
        return ChevronRightIconUrl;
    },
    CopyIconUrl: function() {
        return CopyIconUrl;
    },
    ErrorIconUrl: function() {
        return ErrorIconUrl;
    },
    FixIconUrl: function() {
        return FixIconUrl;
    },
    InconsolataFont: function() {
        return InconsolataFont;
    },
    Style: function() {
        return Style;
    },
    checkIfInGadgetIFrame: function() {
        return checkIfInGadgetIFrame;
    },
    overlayTemplate: function() {
        return overlayTemplate;
    },
    patchOverlay: function() {
        return patchOverlay;
    }
});
...
const ErrorIconUrl = `data:image/svg+xml;base64,${btoa(`<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path
    fill-rule="evenodd"
    clip-rule="evenodd"
...
const FixIconUrl = `data:image/svg+xml;base64,${btoa(`<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path
...
`)}`;
const CopyIconUrl = `data:image/svg+xml;base64,${btoa(`<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path
...
const ChevronRightIconUrl = `data:image/svg+xml;base64,${btoa(`<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const getErrorMessage = (overlayStyle)=>{
    let errorMessage = "";
    switch(overlayStyle.style){
        case "backendError":
            {
...
const overlayTemplate = (overlay)=>{
    ...
    const getErrorTitleText = (overlayStyle)=>{
        switch(overlayStyle.style){
            case "backendError":
    ...
    const maybeGetTraceIdLinkComponent = (overlayStyle)=>{
      ...
      <div class="error-trace-id-content-box">
        <div class="error-trace-id-content">
            <div style="color: var(--gray-alpha-alpha600, rgba(0, 0, 0, 0.67)); font-size: 14px; font-weight: 500; line-height: 20px; word-wrap: break-word">traceID: ${overlayStyle.traceId}</div>
        </div>
        <img src="${ChevronRightIconUrl}" alt="Chevron right icon" width="16" height="16" />
      </div>
      ${maybeGetErrorMessageComponent(overlayStyle)}`;
    };
    const maybeGetErrorMessageComponent = (overlayStyle)=>{
    ...
    const maybeGetErrorDetailsContentComponent = (overlayStyle)=>{
    ...
    const maybeGetStackTraceComponent = (overlayStyle)=>{
...
<div class="window">
  ...
  ${overlay.style === "backendError" ? /* html */ `
...
class ErrorOverlay extends HTMLElement {
    ...
    constructor(err){
        super(), _define_property(this, "root", void 0);
        ...
        if (gadgetFolderMatch) {
            ...
            const locationQueryParam = errorLine !== undefined && errorColumn !== undefined ? `?startLineNumber=${errorLine}&startColumn=${errorColumn}&force=1` : "";
            if (window.self !== window.top) {
                ...
                if (filePathButton) {
                    filePathButton.addEventListener("click", ()=>{
                        checkIfInGadgetIFrame().then((isInGadgetIFrame)=>{
                            if (isInGadgetIFrame) {
                                window.parent.postMessage({
                                    type: "gadget:openFile",
                                    filePath: filePathForLink
                                }, "*");
                            } else {
                                window.open(`${window.location.origin}/edit/${currentEnvironment}/files/${filePathForLink}${locationQueryParam}`, window.self === window.top ? "_top" : "_blank");
                            }
                        }).catch((err)=>{
                ...
                fileLink = this.createLink(`${fileText}`, `${window.location.origin}/edit/${currentEnvironment}/files/${filePathForLink}${locationQueryParam}`);
            }
        }
...
function getOverlayCode(environment) {
    return `
...
function patchOverlay(code, environment) {
